...
# ---------------------------------------------------------------------------------------------------------------------
# THIS CONFIGURATION REQUIRES TERRAFORM VERSION 0.12+
# ---------------------------------------------------------------------------------------------------------------------
...
resource "aws_security_group_rule" "allow_server_rpc_inbound" {
  count            = (length(var.allowed_inbound_cidr_blocks) + length(var.allowed_inbound_ipv6_cidr_blocks)) >= 1 ? 1 : 0
  type             = "ingress"
  from_port        = var.server_rpc_port
  to_port          = var.server_rpc_port
  protocol         = "tcp"
  cidr_blocks      = var.allowed_inbound_cidr_blocks
  ipv6_cidr_blocks = var.allowed_inbound_ipv6_cidr_blocks
...
module "client_security_group_rules" {
  ...
  allowed_inbound_ipv6_cidr_blocks     = var.allowed_inbound_ipv6_cidr_blocks
